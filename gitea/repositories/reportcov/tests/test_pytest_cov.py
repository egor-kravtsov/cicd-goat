...
def test_annotate(testdir):
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "--cov-report=annotate", script
    )
...
def test_annotate_output_dir(testdir):
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "--cov-report=annotate:" + DEST_DIR, script
    )
...
def test_html(testdir):
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "--cov-report=html", script
    )
...
def test_html_output_dir(testdir):
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "--cov-report=html:" + DEST_DIR, script
    )
...
def test_xml_output_dir(testdir):
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "--cov-report=xml:" + REPORT_NAME, script
    )
...
def test_term_output_dir(testdir):
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "--cov-report=term:" + DEST_DIR, script
    )

    result.stderr.fnmatch_lines(
        [
            '*argument --cov-report: output specifier not supported for: "term:%s"*'
            % DEST_DIR,
        ]
    )
    assert result.ret != 0
...
def test_central_nonspecific(testdir, prop):
    ...
    testdir.tmpdir.join(".coveragerc").write(prop.fullconf)
    result = testdir.runpytest(
        "-v", "--cov", "--cov-report=term-missing", script, *prop.args
    )
...
"""
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "--cov-report=term-missing", script
    )
...
"""
    ...
    monkeypatch.setitem(
        os.environ,
        "PYTHONPATH",
        os.pathsep.join([os.environ.get("PYTHONPATH", ""), "aliased"]),
    )
    result = testdir.runpytest(
        "-v",
        "-s",
...
"""
    ...
    result = testdir.runpytest(
        "-v", "-s", "--cov=mod", "--cov-branch", script, *opts.split()
    )
...
"""
    ...
    result = testdir.runpytest(
        "-v", "--cov", "--cov-report=term-missing", parent_script
    )
...
@pytest.mark.skipif(
    'sys.platform == "win32" or platform.python_implementation() == "PyPy"'
)
...
"""
    ...
    result.stdout.fnmatch_lines(
        ["test_dist_combine_racecondition* 0 * 100%*", "*1000 passed*"]
    )
...
def test_central_subprocess(testdir):
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % scripts.dirpath(), "--cov-report=term-missing", parent_script
    )
...
def test_central_subprocess_change_cwd(testdir):
    scripts = testdir.makepyfile(
        parent_script=SCRIPT_PARENT_CHANGE_CWD, child_script=SCRIPT_CHILD
    )
    parent_script = scripts.dirpath().join("parent_script.py")
    testdir.makefile(
        "",
...
def test_invalid_coverage_source(testdir):
    ...
    result = testdir.runpytest(
        "-v", "--cov=non_existent_module", "--cov-report=term-missing", script
    )
...
@pytest.mark.skipif(
    "sys.version_info[0] < 3", reason="no context manager api on Python 2"
)
@pytest.mark.skipif(
    'sys.platform == "win32"', reason="multiprocessing support is broken on Windows"
)
@pytest.mark.skipif(
    'platform.python_implementation() == "PyPy"', reason="often deadlocks on PyPy"
)
@pytest.mark.skipif(
...
@pytest.mark.skipif(
    'sys.version_info[0] > 2 and platform.python_implementation() == "PyPy"',
...
"""
    ...
    result = testdir.runpytest("-v", "--cov", "--cov-report=term-missing", script)
...
@pytest.mark.skipif(
    'sys.platform == "win32"', reason="SIGTERM isn't really supported on Windows"
)
...
'''
    ...
    result = testdir.runpytest(
        "-vv", "--cov=%s" % script.dirpath(), "--cov-report=term-missing", script
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "signal.signal(signal.SIGBREAK, signal.SIG_DFL); cleanup_on_signal(signal.SIGBREAK)",
            "87%   21-22",
        ),
...
@pytest.mark.parametrize(
    ...
    [
        (
            "signal.signal(signal.SIGTERM, signal.SIG_DFL); cleanup_on_sigterm()",
            "88%   18-19",
        ),
...
@pytest.mark.skipif(
    'sys.platform == "win32"', reason="SIGINT is subtly broken on Windows"
)
...
def test_cover_looponfail(testdir, monkeypatch):
    ...
    with testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "--looponfail", script
    ) as process:
...
def test_coveragerc_dist(testdir):
    ...
    assert result.ret == 0
    result.stdout.fnmatch_lines(["test_coveragerc_dist* %s" % EXCLUDED_RESULT])
...
@pytest.mark.parametrize(
    "report_option", ["term-missing:skip-covered", "term:skip-covered"]
)
def test_skip_covered_cli(testdir, report_option):
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "--cov-report=%s" % report_option, script
    )
    assert result.ret == 0
...
def test_skip_covered_coveragerc_config(testdir):
    ...
    script = testdir.makepyfile(SKIP_COVERED_TEST)
    result = testdir.runpytest(
        "-v", "--cov-config=coveragerc", "--cov=%s" % script.dirpath(), script
    )
    assert result.ret == 0
    result.stdout.fnmatch_lines([SKIP_COVERED_RESULT])
...
def test_dist_boxed(testdir):
    ...
    result = testdir.runpytest(
        "-v", "--assert=plain", "--cov=%s" % script.dirpath(), "--boxed", script
    )
...
def test_default_output_setting(testdir):
    ...
    result = testdir.runpytest("-v", "--cov=%s" % script.dirpath(), script)
...
def test_disabled_output(testdir):
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "--cov-report=", script
    )
...
"""
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "-n", "1", max_worker_restart_0, script
    )
    assert result.ret == 0
    assert glob.glob(str(testdir.tmpdir.join("some/special/place/coverage-data*")))
...
def test_append_coverage(testdir, opts, prop):
    ...
    testdir.tmpdir.join(".coveragerc").write(prop.fullconf)
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), script, *opts.split() + prop.args
    )
    result.stdout.fnmatch_lines(
        [
...
def test_do_not_append_coverage(testdir, opts, prop):
    ...
    script2 = testdir.makepyfile(test_2=prop.code2)
    result = testdir.runpytest(
        "-v", "--cov=%s" % script2.dirpath(), script2, *opts.split() + prop.args
    )
    result.stdout.fnmatch_lines(
        [
            "test_1* 0%",
...
def test_double_cov(testdir):
    script = testdir.makepyfile(SCRIPT_SIMPLE)
    result = testdir.runpytest(
        "-v", "--assert=plain", "--cov", "--cov=%s" % script.dirpath(), script
    )
...
def test_cov_reset(testdir):
    script = testdir.makepyfile(SCRIPT_SIMPLE)
    result = testdir.runpytest(
        "-v", "--assert=plain", "--cov=%s" % script.dirpath(), "--cov-reset", script
    )
...
def test_contexts(testdir, opts):
    ...
    result = testdir.runpytest(
        "-v", "--cov=%s" % script.dirpath(), "--cov-context=test", script, *opts.split()
    )
    assert result.ret == 0
    result.stdout.fnmatch_lines(
        [
