            ...
            if pipeline_id is not None:
                for job_entry in pipeline.jobs.list():
                    ...
                    if job.status == "failed":
                        print(
                            f"Project: {project.name}, Job: {job.name}, Trace:\n{job.trace().decode()}"
                        )
...
def test_gryphon():
    process = run(
        [
            ...
            "-m",
            "build",
            os.path.abspath("tests/data/pygryphon"),
        ]
    )
    assert process.stderr is None
    # Alice
    gl = gitlab.Gitlab(
        "http://localhost:4000", private_token="998b5802ec365e17665d832f3384e975"
    )
    ...
    process = run(
        [
            ...
            "--config-file",
            os.path.abspath("tests/data/pygryphon/.pypirc"),
        ]
    )
    assert process.stderr is None
    # Root
    gl = gitlab.Gitlab(
        "http://localhost:4000", private_token="60b6c7ba41475b2ebdded2c0d3b079f0"
    )
    ...
    sleep(5)
    process = run(
        "docker exec -e DOCKER_HOST=localhost:2376 prod docker logs web",
        stdout=PIPE,
        shell=True,
    )
    assert process.stderr is None
