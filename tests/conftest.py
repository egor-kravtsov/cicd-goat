...
class GiteaApiClient:
    ...
    def __init__(self):
        GiteaApiClient.token = requests.post(
            f"{GITEA_API_BASE}/users/thealice/tokens",
    ...
    def create_fork(self, owner, repo):
        ...
        for repo_dict in res.json():
            ...
            res = self.post(
                f"/repos/{owner}/{repo}/forks", json={"organization": FORK_ORG}
            )
...
class JenkinsClient(Jenkins):
    def post(self, endpoint, data=None, **kwargs):
        return self.requester.post_url(
            f"{self.baseurl}{endpoint}", data=data, timeout=120, **kwargs
        )
    ...
    def find_in_last_build_console(self, job_path, string, start_job=True):
        ...
        results = [
            search_last_build(job) for name, job in self.get_jobs() if job_name in name
        ]
        if [result for result, console in results if result]:
            return True
        print(
            "--------------------------\n".join(
                [console for result, console in results if console]
            )
        )
        return False
...
def jenkins_client():
    return JenkinsClient(
        "http://localhost:8080", username="alice", password="alice", use_crumb=True
    )
