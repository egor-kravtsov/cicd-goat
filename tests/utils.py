...
def branch_and_replace_file_content(
    repo, new_branch_name, relative_file_path, replace_tuples, commit=True, push=True
):
...
def branch_and_write_file(
    repo, new_branch_name, relative_file_path, content, commit=True, push=True
):
    previous_branch = repo.head.reference
    if new_branch_name not in repo.heads:
        repo.create_head(new_branch_name, "HEAD")
