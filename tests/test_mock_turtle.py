...
def test_mock_turtle(gitea_client, jenkins_client):
    ...
    branch_and_replace_file_content(
        repo, new_branch_name, "version", replace_tuples, commit=False, push=False
    )
    replace_tuples = [(REMOVE_WORDS, "")]
    branch_and_replace_file_content(
        repo, new_branch_name, "README.md", replace_tuples, commit=False, push=False
    )
    replace_tuples = [("withCredentials", WITH_CREDENTIALS)]
    branch_and_replace_file_content(
        repo, new_branch_name, "Jenkinsfile", replace_tuples
    )
