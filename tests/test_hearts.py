...
def test_hearts():
    jenkins_client = JenkinsClient(
        "http://localhost:8080", username="knave", password="rockme", use_crumb=True
    )
    config = {
        "name": "test",
    ...
    assert (
        "Accepted password for agent" in result.stdout
        or "Accepted password for agent" in result.stderr
    )
