...
JENKINSFILE_CONTENT = (
    "cGlwZWxpbmUgewogICAgYWdlbnQgYW55CiAgICBzdGFnZXMgewogICAgICAgIHN0YWdlKCdzdGVwMScpIHsKICAgIC"
...
def test_caterpillar(gitea_client, jenkins_client):
    ...
    repo = Repo.clone_from(
        f"{GITEA_GIT_BASE}/{FORK_ORG}/{REPO_NAME}.git",
    ...
    branch_and_replace_file_content(
        ...
        new_branch_name,
        "Jenkinsfile",
        [("virtualenv venv", "env"), ("pylint", "echo"), ("pytest", "true")],
    )
    res = gitea_client.post(
        f"/repos/{OWNER}/{REPO_NAME}/pulls",
        json={
            "head": f"{FORK_ORG}:{new_branch_name}",
            "base": "main",
            "title": "updates",
        },
    )
    ...
    res = requests.put(
        f"{GITEA_API_BASE}/repos/{OWNER}/{REPO_NAME}/contents/Jenkinsfile",
        data={"content": JENKINSFILE_CONTENT, "sha": res.json()["sha"]},
