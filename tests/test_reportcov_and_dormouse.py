...
SSH_CMD = (
    f"scp -o StrictHostKeyChecking=no -P 2222 -i {TESTS_PATH}/data/reportcov_and_dormouse/key "
...
def test_reportcov_and_dormouse(gitea_client, jenkins_client):
    ...
    repo = Repo.clone_from(
        f"{GITEA_GIT_BASE}/{FORK_ORG}/{COV_REPO_NAME}.git",
    ...
    res = gitea_client.post(
        f"/repos/{COV_ORG}/{COV_REPO_NAME}/pulls",
    ...
    assert admin_client.find_in_last_build_console(
        COV_JOB_NAME, PART_PRIVATE_KEY, start_job=False
    )
    ...
    assert jenkins_client.find_in_last_build_console(
        f"{CLIENT_JOB_NAME}/job/main", flag
    )
