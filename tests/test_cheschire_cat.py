...
def test_cheshire_cat(gitea_client, jenkins_client):
    admin_jenkins_client = JenkinsClient(
        "http://localhost:8080",
        username="admin",
        password="ciderland5#",
        use_crumb=True,
    ...
    assert (
        b"Only build jobs with label expressions matching this node"
        in admin_jenkins_client.get("/computer/(built-in)/configure").content
    )
    repo = Repo.clone_from(
        f"{GITEA_GIT_BASE}/{OWNER}/{REPO_NAME}.git",
    ...
    branch_and_replace_file_content(
        repo, new_branch_name_one, "Jenkinsfile", replace_tuples
    )
    res = gitea_client.post(
        f"/repos/{OWNER}/{REPO_NAME}/pulls",
        json={"head": new_branch_name_one, "base": "main", "title": "updates"},
    )
    assert res.status_code == 201
    assert jenkins_client.find_in_last_build_console(
        JOB_NAME, "6B31A679-6D70-469D-9F8D-6D6E80B3C29C"
    )
